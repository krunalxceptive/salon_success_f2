...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/apply-promo-code", requireAuth, async (req, res) => {
    try {
      ...
      if (code.toUpperCase() === "CLIENT6FREE") {
        ...
        console.log(
          `CLIENT6FREE promo code used by user: ${user?.email} - 6 months free access until ${sixMonthsEndDate}`
        );
        ...
        return res.json({
          success: true,
          message:
            "Congratulations! You now have 6 months of free access to all business tools.",
        });
      } else {
        return res.status(400).json({
          success: false,
  ...
  app.post("/api/test-update-user-status", async (req, res) => {
    try {
      ...
      const user = await storage.updateSubscriptionStatus(
        1,
        subscriptionStatus,
  ...
  app.post("/api/hourly-rate-calculations", requireAuth, async (req, res) => {
    try {
      const userId = req.session.userId!;
      console.log(
        "NEW AUTH: Creating hourly rate calculation for user ID:",
        userId
      );
  ...
  app.get("/api/hourly-rate-calculations", requireAuth, async (req, res) => {
    try {
      ...
      const calculations = await storage.getHourlyRateCalculationsByUserId(
        userId
      );
      console.log(
        "Found calculations:",
  ...
  app.get(
    "/api/hourly-rate-calculations/latest",
    requireAuth,
    async (req, res) => {
      try {
        const userId = req.session.userId!;
        const calculation = await storage.getLatestHourlyRateCalculation(
          userId
        );
  ...
  app.get("/api/metrics", requireAuth, async (req, res) => {
    try {
      ...
      const latestHourlyRate = await storage.getLatestHourlyRateCalculation(
        userId
      );
      ...
      const avgProfitMargin =
        treatments.length > 0
          ? treatments.reduce((sum, t) => sum + parseFloat(t.profitMargin), 0) /
            treatments.length
          : 0;
      ...
      const monthlyExpenses = await storage.getExpensesByUserIdAndDateRange(
        userId,
        firstDayOfMonth,
        currentMonth
      );
      ...
      const estimatedMonthlyRevenue =
        treatments.length > 0
          ? treatments.reduce(
              (sum, t) => sum + parseFloat(t.price.toString()),
              0
            ) * 0.7 // Assuming 70% booking rate
          : 0;
  ...
  app.post("/api/weekly-incomes", requireAuth, async (req, res) => {
    try {
      ...
      if (!validation.success) {
        console.error(
          "Weekly income validation failed:",
      ...
      const weeklyIncome = await storage.createOrUpdateWeeklyIncome(
        validation.data
      );
  ...
  app.get("/api/stripe-test", async (req, res) => {
    try {
      ...
      console.log(
        ...
        process.env.VITE_STRIPE_PUBLIC_KEY?.substring(0, 8)
      );
      ...
      const price = await testStripe.prices.retrieve(
  ...
  app.post("/api/create-subscription", requireAuth, async (req, res) => {
    try {
      ...
      if (!process.env.STRIPE_PRICE_ID) {
        return res
          .status(500)
          .json({ message: "Stripe price ID not configured" });
      }
      ...
      return res.status(400).json({
        error: {
          message:
            error.message ||
            "Subscription creation failed. Please check Stripe configuration.",
        },
      });
    }
  });
 ...
 app.get("/api/subscription-status", async (req, res) => {
  ...
  res.set("Expires", "0");              // ensure immediate expiration
    ...
    if (stripeSubscriptionId) {
      ...
      try {
        ...
        await storage.updateSubscriptionStatus(
          user.id,
          subscription.status,
          !isNaN(endDate.getTime()) ? endDate : null
        );
      } catch (err) {
  ...
  app.post("/api/create-promo-code", async (req, res) => {
    try {
      const testStripe = new Stripe(
        "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",
        {
          apiVersion: "2025-05-28.basil",
        }
      );
  ...
  app.post("/api/email-report", requireAuth, async (req, res) => {
    try {
      ...
      const treatments = await storage.getTreatmentsByUserId(userId);
      const expenses = await storage.getExpensesByUserId(userId);
      const hourlyRateCalc = await storage.getLatestHourlyRateCalculation(
        userId
      );
      const businesses = await storage.getBusinessesByUserId(userId);
      ...
      const totalMonthlyRevenue =
        weeklyIncomes.reduce(
          (sum, w) => sum + parseFloat(w.weeklyTotal.toString()),
          0
        ) * 4.33;
          ...
          <div class="metrics">
            <div class="metric-card">
              <div class="metric-value">£${
                hourlyRateCalc?.calculatedRate || "0"
              }/hr</div>
            ...
            <table>
              ...
              <tbody>
                ${treatments
                  ...
                  <tr>
                    <td>${t.name}</td>
                    <td>£${parseFloat(t.price?.toString() || "0").toFixed(
                      2
                    )}</td>
                    <td>${t.duration || 0} min</td>
                    <td>${parseFloat(t.profitMargin?.toString() || "0").toFixed(
                      1
                    )}%</td>
                  </tr>
                `
      ...
      try {
        ...
        if (emailSent) {
          ...
          res.json({
            success: true,
            message:
              "Email service unavailable. Opening your email client instead.",
            fallback: true,
            emailData: {
              subject: `Business Report - ${new Date().toLocaleDateString()}`,
              body: `Please find your business performance report below:\n\nKey Metrics:\n- Current Hourly Rate: £${
                hourlyRateCalc?.calculatedRate || "0"
              }/hr\n- Monthly Revenue: £${totalMonthlyRevenue.toFixed(
                2
              )}\n- Total Treatments: ${
                treatments.length
      ...
      res
        .status(500)
        .json({ success: false, message: "Failed to generate email report" });
    }
  });
